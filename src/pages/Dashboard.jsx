...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [projectsRes, apiNodesRes, webNodesRes, jenkinsStatusRes] =
        await Promise.all([
          axios
            .get(`${import.meta.env.VITE_BACKEND_URL}projects`)
            .catch(() => ({ data: [] })),
          axios
            .get(`${import.meta.env.VITE_BACKEND_URL}preview-services`)
            .catch(() => ({ data: { services: [] } })),
          axios
            .get(`${import.meta.env.VITE_BACKEND_URL}preview-nodes`)
            .catch(() => ({ data: { services: [] } })),
          axios
            .get(`${import.meta.env.VITE_BACKEND_URL}jenkins/test-connection`)
            .catch(() => ({ data: { connected: false } })),
        ]);

      const projects = projectsRes.data || [];
      const apiPayload = apiNodesRes.data || {};
      const webPayload = webNodesRes.data || {};
      const backendServices =
        apiPayload.services ??
        apiPayload.apiServices ??
        apiPayload.backendServices ??
        [];
      const frontendServices =
        webPayload.services ??
        webPayload.webServices ??
        webPayload.frontendServices ??
        [];

      // Filter out deleted services
      const activeBackendServices = backendServices.filter(
        (s) => !s.is_deleted,
      );
      const activeFrontendServices = frontendServices.filter(
        (s) => !s.is_deleted,
      );
      const allServices = [...activeBackendServices, ...activeFrontendServices];

      // Calculate stats
      const activeBuilds = allServices.filter(
        (s) => s.build_status === "building",
      ).length;
      const successfulBuilds = allServices.filter(
        (s) => s.build_status === "success",
      ).length;
      const failedBuilds = allServices.filter(
        (s) => s.build_status === "failed",
      ).length;
      ...
      activeBackendServices.forEach((service, index) => {
        if (service.build_status && service.build_status !== "pending") {
          ...
          recentBuildsList.push({
            ...
            duration:
              (service.last_build_at || service.last_build_date) &&
              service.created_at_build
                ? formatDuration(
                    service.created_at_build,
                    service.last_build_at || service.last_build_date,
                  )
                ...
                systemStatus.map((item, index) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (item.projectId) {
                        navigate(
                          `/projects/${item.projectId}/nodes/${item.id}`,
                        );
                      } else {
                        navigate("/projects");
                      }
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      ></div>
                      <div>
                        <Text strong className="text-black dark:text-white">
                          {item.title}
                        </Text>
                        <br />
